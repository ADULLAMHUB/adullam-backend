 ...
 * Log AI interaction
 */
async function logAIInteraction(userId, action, parameters, result, status) {
    try {
        await db_1.default.aIInteraction.create({
            data: {
                userId,
                action,
                parameters,
 ...
 */
async function searchMentors(query, userId) {
    ...
    if (mentors.length === 0 && lowercaseQuery.includes('mentor')) {
        const allMentors = await db_1.default.user.findMany({
            where: { role: 'MENTOR' },
            include: {
                mentees: true
            }
 ...
 */
async function connectToMentor(menteePhone, mentorName) {
    try {
        // Find mentor by name
        ...
        // Get mentee
        const mentee = await db_1.default.user.findFirst({
        ...
        const existingConnection = await db_1.default.connection.findFirst({
            where: {
                mentorId: mentor.id,
                menteeId: mentee.id,
            }
...
async function generateAIResponse(userMessage, phoneNumber) {
    try {
        ...
        const wantsToConnect = connectKeywords.some(keyword => userMessage.toLowerCase().includes(keyword));
        if (wantsToConnect) {
            ...
            await logAIInteraction(user.id, 'CONNECT_TO_MENTOR', { mentorName: possibleName }, { success: !connectResult.includes("couldn't find") }, connectResult.includes("couldn't find") ? 'FAILED' : 'SUCCESS');
        ...
        if (isAboutMentors) {
            ...
            if (mentorResponse) {
                await logAIInteraction(user.id, 'SEARCH_MENTORS', { query: userMessage }, { responseLength: mentorResponse.length }, 'SUCCESS');
                return mentorResponse;
            }
        }
        // Get recent interactions for context
        const recentInteractions = await db_1.default.aIInteraction.findMany({
...
You can help users connect with mentors. If they want to connect with someone, guide them through the process.`;
        ...
        await logAIInteraction(user.id, 'CHAT', { message: userMessage }, { response: aiResponse.substring(0, 100) }, 'SUCCESS');
        return aiResponse;
    }
    catch (error) {
        ...
        try {
            const user = await getOrCreateUser(phoneNumber);
            await logAIInteraction(user.id, 'ERROR', { message: userMessage, error: String(error) }, null, 'FAILED');
        }
        catch { }
...
app.get("/health", async (req, res) => {
    try {
        ...
        const mentorCount = await db_1.default.user.count({ where: { role: 'MENTOR' } });
        const connectionCount = await db_1.default.connection.count();
        res.json({
            status: "healthy",
...
app.delete("/conversations/:phone", async (req, res) => {
    try {
        ...
        if (user) {
            ...
            await db_1.default.aIInteraction.deleteMany({
                where: { userId: user.id }
            });
        }
